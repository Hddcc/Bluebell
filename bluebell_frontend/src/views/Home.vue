          ...
          <div class="post">
            <a class="vote">
              <span class="iconfont icon-up"
                    :class="{ 'voted': post.vote_status == 1 }"
                    @click="vote(post, 1)"></span>
            </a>
...
export default {
  ...
  methods: {
    ...
    vote(post, direction) {
      ...
      // If the user clicks the same vote again, it's a cancellation
      if (post.vote_status === direction) {
        newDirection = 0;
      }
      this.$axios({
        method: "post",
        ...
        .then(response => {
          if (response.code == 1000) {
            ...
            this.getPostList(); // Refresh the list
          } else {
            alert(response.msg); // Notify user about failure
            console.log(response.msg);
          }
        })
        .catch(error => {
          console.log(error);
          alert("投票失败，请稍后再试或检查网络连接。"); // Generic error message
        });
    }
  },
  mounted: function() {
